...
export default function ProductCard({
  ...
  const was = compareAtOf(price, p.tags);

  const priceEl = price != null
    ? <><span>EGP</span> {price}{was != null && <s className="price-was">EGP {was}</s>}</>
    : <em>Select branch</em>;
    ...
    return (
      <article className={`pcard pcx pcx-h pcx-${size}`}>
        ...
        <div className="pcard-info">
          ...
          {showPrice && <span className={`product-price${was != null ? ' is-deal' : ''}`}>{price != null ? <>EGP {price}{was != null && <s className="price-was">EGP {was}</s>}</> : 'Select branch'}</span>}
      ...
      <div className="mw-body">
        ...
        {(showPrice || showAdd) && (
          <div className="mw-foot">
            {showPrice && <span className={`mw-price${was != null ? ' is-deal' : ''}`}>{priceEl}</span>}
            {showAdd && (
